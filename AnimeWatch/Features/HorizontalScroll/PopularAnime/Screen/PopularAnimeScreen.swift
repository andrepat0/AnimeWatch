...
struct PopularAnimeScreen: View {
    ...
    @StateObject private var vm = PopularAnimeViewModelImpl(service: PopularAnimeServiceImpl())
    
    var body: some View {
        
       ScrollView(.horizontal, showsIndicators: false){
         LazyHStack{
             ForEach(vm.popularAnimes,id: \.title){ anime in
              CardView(item:anime).shadow(color: Color.black.opacity(0.2),radius:5,x: 5,y: 5)
         }
       }
    }.padding(20)
    .task{
            await vm.getPopularAnime()
    }
   }
}

struct PopularAnimeScreen_Previews: PreviewProvider {
