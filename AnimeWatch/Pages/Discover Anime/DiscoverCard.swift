...
struct DiscoverCardView: View {
    ...
    var body: some View {
        NavigationLink(destination: AnimeDetailJikanScreen(anime_id: data.mal_id ?? 0, data: nil)) {
                ...
                VStack(alignment: .leading) {
                    HStack{
                        ...
                        VStack(alignment: .center,spacing: 5){
                            StarAssignment(score: data.score)
                                .resizable()
                                .aspectRatio(contentMode: .fit)
                                .frame(width: 25, height: 20)
                            Text("\(data.score)%").font(.custom(FontsManager.Montserrat.bold, size: 14)).foregroundColor(Color("white"))
